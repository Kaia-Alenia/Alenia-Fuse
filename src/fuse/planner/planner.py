...
class OperationPlanner:
    ...
    def preflight_check(self, plan: OperationPlan, timeout: float = 5.0) -> bool:
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(tmp_path) and os.path.getsize(tmp_path) > 0:
                return True
            plan.is_valid = False
            plan.error_reason = "Preflight check failed. FFmpeg could not process this format combination."
            return False
        except subprocess.TimeoutExpired:
            return True
        except Exception as exc:
            plan.is_valid = False
            plan.error_reason = f"Preflight execution failed: {exc}"
            return False
        finally:
    ...
    def plan_convert(self, media, target_format: str, output_path: str) -> OperationPlan:
        ...
        if output_suffix not in allowed_suffixes:
            ...
            plan.error_reason = f"The output filename must use a valid '{fmt}' extension ({expected}). Received '{output_suffix or '[no extension]' }'."
            return plan
        ...
        if not self.registry.supports_format(fmt):
            plan.is_valid = False
            plan.error_reason = f"The target format '{fmt}' cannot represent the required input features, or is not supported."
            return plan
        if not self._check_image_semantics(media, fmt, plan):
            return plan

        if self._can_stream_copy(media, fmt):
    ...
    def plan_compress(self, media, output_path: str, quality: str = "balanced", target_size_mb: Optional[float] = None) -> OperationPlan:
        ...
        if target_size_mb and media.duration > 0:
            ...
            video_bitrate = int((target_bits / media.duration) * 0.9)
            plan.strategy = "two_pass"
            plan.args = ["-i", media.path, "-c:v", "libx264", "-b:v", str(video_bitrate), "-c:a", "aac", "-b:a", "128000", output_path]
            return plan
        strategy = COMPRESS_STRATEGIES.get(quality, COMPRESS_STRATEGIES["balanced"])
        plan.args = ["-i", media.path, "-c:v", "libx264", "-crf", str(strategy["crf"]), "-preset", strategy["preset"], "-c:a", "aac", "-b:a", strategy["audio_bitrate"], output_path]
        return plan
    ...
    def plan_video_op(self, operation: str, media, output_path: str, **kwargs) -> OperationPlan:
        ...
        args = ["-i", media.path]
        if operation == "resize":
            args += ["-vf", f"scale={kwargs.get('width', 0)}:{kwargs.get('height', 0)}", "-c:a", "copy", output_path]
        elif operation == "crop":
            args += ["-vf", f"crop={kwargs.get('width', 0)}:{kwargs.get('height', 0)}:{kwargs.get('x', 0)}:{kwargs.get('y', 0)}", "-c:a", "copy", output_path]
        elif operation == "rotate":
            degrees = kwargs.get("degrees", 90)
            rotation_map = {90: "transpose=1", 180: "transpose=1,transpose=1", 270: "transpose=2", -90: "transpose=2"}
            args += ["-vf", rotation_map.get(int(degrees), f"rotate={degrees}*PI/180"), "-c:a", "copy", output_path]
        elif operation == "fps":
            args += ["-vf", f"fps={kwargs.get('fps', 30)}", "-c:a", "copy", output_path]
        elif operation == "speed":
            factor = kwargs.get("factor", 1.0)
            vf = f"setpts={1.0 / factor:.4f}*PTS"
            af = f"atempo={factor:.4f}" if 0.5 <= factor <= 2.0 else (f"atempo=2.0,atempo={factor / 2.0:.4f}" if factor > 2.0 else f"atempo=0.5,atempo={factor * 2.0:.4f}")
            args += ["-vf", vf, "-af", af, output_path]
        elif operation in ("trim", "cut"):
            args += ["-ss", str(kwargs.get("start", "00:00:00"))]
            if kwargs.get("end"):
                args += ["-to", str(kwargs["end"])]
        ...
        elif operation in ("thumbnail", "frame"):
            args += ["-ss", str(kwargs.get("timestamp", "00:00:05")), "-vframes", "1", output_path]
        elif operation == "gif":
            args = ["-i", media.path, "-ss", str(kwargs.get("start", "00:00:00")), "-t", str(kwargs.get("duration", 5)), "-vf", f"fps={kwargs.get('fps', 10)},scale={kwargs.get('width', 480)}:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse", output_path]
        elif operation == "remux":
            args += ["-c", "copy", output_path]
        elif operation == "optimize":
            args += ["-c:v", "libx264", "-preset", kwargs.get("preset", "medium"), "-crf", str(kwargs.get("crf", 23)), "-c:a", "aac", "-b:a", "128k", output_path]
            ...
            if not watermark_path:
                ...
                return plan
            positions = {"topleft": "x=10:y=10", "topright": "x=W-w-10:y=10", "bottomleft": "x=10:y=H-h-10", "bottomright": "x=W-w-10:y=H-h-10", "center": "x=(W-w)/2:y=(H-h)/2"}
            args = ["-i", media.path, "-i", watermark_path, "-filter_complex", f"[0:v][1:v]overlay={positions.get(kwargs.get('position', 'bottomright'), positions['bottomright'])}", "-c:a", "copy", output_path]
            ...
            args += ["-c", "copy"]
            for key, option in (("title", "title"), ("artist", "artist"), ("album", "album"), ("year", "date")):
                if kwargs.get(key):
                    args += ["-metadata", f"{option}={kwargs[key]}"]
            args += [output_path]
        else:
            plan.is_valid = False
    ...
    def plan_audio_op(self, operation: str, media, output_path: str, **kwargs) -> OperationPlan:
        ...
        if operation == "volume":
            ...
            vol_filter = f"volume={1.0 + float(value.rstrip('%')) / 100:.4f}" if value.endswith("%") else f"volume={value}"
            ...
            if fade_type == "in":
                ...
                start = max(0, media.duration - float(duration)) if media.duration > 0 else 0
            ...
            atempo = f"atempo={factor:.4f}" if 0.5 <= factor <= 2.0 else (f"atempo=2.0,atempo={factor / 2.0:.4f}" if factor > 2.0 else f"atempo=0.5,atempo={factor * 2.0:.4f}")
