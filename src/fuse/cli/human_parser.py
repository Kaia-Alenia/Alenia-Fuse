"""
Human-syntax parser for Alenia Fuse CLI.
...
def parse_human_syntax(tokens: list[str]) -> ParsedCommand | None:
    ...
    if cmd == "convert" and len(tokens) >= 3:
        if tokens[2].lower() == "to" and len(tokens) >= 4:
            # pass format as the output so the handler calculates smart routing
            input_file = tokens[1]
            target_fmt = f".{tokens[3].lstrip('.')}"
            return ParsedCommand("convert", {"input": input_file, "output": target_fmt})
