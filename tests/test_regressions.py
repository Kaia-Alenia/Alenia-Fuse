...
from alenia_porter.planner.planner import OperationPlanner, OperationPlan
...
def test_input_equals_output_rejected(tmp_path):
    ...
    with pytest.raises(IncompatibleOperationError, match="Input and output paths cannot be identical"):
        op.run()
